...
# Generic wrapper for AES-128 CBC encrypting
def cbcEncrypt(fileContents, key = None):
    global globalKeyVariable
    fileContentsBytes = bytes(fileContents, 'utf-8')
    lengthFileContents = len(fileContentsBytes)
    if key == None:
    ...
    encrypt = CBC_CIPHER.encrypt(fileContentsBytes)
    encrypt = encrypt[:lengthFileContents]                # remove padded bytes before writing to file
    return encrypt
...
def main():
    ...
    # change this to AES stuffs
    encrypt = ECB_CIPHER.encrypt(fileContents)

    # remove padded bytes before writing to file
    encrypt = encrypt[:lengthFileContents]

    encryptedFileName = fileToBeEncrypted + "_ECB.bmp"
